...
const Sessions = () => {
  ...
  const handlePitchClick = (e) => {
    ...
    if (selectedTool === 'line') {
      if (!lineStartPoint) {
        ...
        // Second click: set the ending point and create the line
        const newLine = {
          id,
          type: 'line',
          points: [
        ...
        setLineStartPoint(null); // Reset the line drawing state
        setSelectedTool(null); // Optionally reset the tool selection
      }
      ...
      if (!squareStartPoint) {
        ...
        // Second click: set the ending point and create the square
        ...
        setSquareStartPoint(null); // Reset the square drawing state
        setSelectedTool(null); // Optionally reset the tool selection
      }
  ...
  // Function to handle mouse movement for temporary line and square
  ...
  // Function to handle mouse up to clear temporary line and square
  const handleMouseUp = () => {
    if (selectedTool === 'line') {
      setTempLine(null);
    }
  ...
  const handleTransformObject = (e, id) => {
    ...
    const updatedObjects = objects.map((obj) => {
      if (obj.id === id) {
        ...
        // Handle other transformable objects if needed
        return {
          ...obj,
          x: node.x(),
          y: node.y(),
