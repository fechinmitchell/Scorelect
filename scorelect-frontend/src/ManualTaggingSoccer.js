...
// Styled Components (reusing from ManualTagging)
...
const SoccerPitchSelector = ({ currentPosition, setCurrentPosition, selectedTeam }) => {
    // Define the aspect ratio to match soccer pitch (105/68)
    ...
    const xScale = dimensions.width / 105;
    const yScale = dimensions.height / 68;
    ...
    return (
      ...
      >
        <Layer>
          {/* Put the renderSoccerPitchElements INSIDE the Layer */}
...
// Main Manual Tagging Soccer Component
const ManualTaggingSoccer = () => {
  ...
  const sport = location.state?.sport || 'Soccer';
  ...
  const [selectedPosition, setSelectedPosition] = useState('forward');
  const [selectedPressure, setSelectedPressure] = useState('0');
  ...
  // Default soccer teams structure
  const defaultTeams = {
    home: {
      name: 'Home Team',
      players: [
        { id: 1, number: 1, name: 'Player 1', position: 'Goalkeeper' },
        { id: 2, number: 2, name: 'Player 2', position: 'Defender' },
        { id: 3, number: 3, name: 'Player 3', position: 'Defender' },
        { id: 4, number: 4, name: 'Player 4', position: 'Defender' },
        ...
        { id: 8, number: 8, name: 'Player 8', position: 'Forward' },
        { id: 9, number: 9, name: 'Player 9', position: 'Forward' },
        { id: 10, number: 10, name: 'Player 10', position: 'Forward' },
        { id: 11, number: 11, name: 'Player 11', position: 'Forward' },
      ]
    },
    away: {
      name: 'Away Team',
      players: [
        { id: 12, number: 1, name: 'Player 12', position: 'Goalkeeper' },
        { id: 13, number: 2, name: 'Player 13', position: 'Defender' },
        { id: 14, number: 3, name: 'Player 14', position: 'Defender' },
        { id: 15, number: 4, name: 'Player 15', position: 'Defender' },
        { id: 16, number: 5, name: 'Player 16', position: 'Midfielder' },
        { id: 17, number: 6, name: 'Player 17', position: 'Midfielder' },
        { id: 18, number: 7, name: 'Player 18', position: 'Forward' },
        { id: 19, number: 8, name: 'Player 19', position: 'Forward' },
        { id: 20, number: 9, name: 'Player 20', position: 'Forward' },
        { id: 21, number: 10, name: 'Player 21', position: 'Forward' },
        { id: 22, number: 11, name: 'Player 22', position: 'Forward' },
      ]
    }
  };
  ...
  // Soccer tag categories and actions
  const tagCategories = [
    {
      id: 'Possession', name: 'Possession', actions: [
        ...
        { id: 'ball_control', name: 'Ball Control' },
    ...
    },
    {
      id: 'Scoring', name: 'Scoring', actions: [
        { id: 'goal', name: 'Goal' },
        { id: 'shot_on_target', name: 'Shot on Target' },
        { id: 'shot_off_target', name: 'Shot off Target' },
    ...
    },
    {
      id: 'Set Pieces', name: 'Set Pieces', actions: [
        { id: 'corner_kick', name: 'Corner Kick' },
        { id: 'free_kick', name: 'Free Kick' },
        { id: 'penalty', name: 'Penalty' },
        { id: 'throw_in', name: 'Throw In' },
        { id: 'goal_kick', name: 'Goal Kick' }
      ]
    },
    {
  ...
  const downloadFrame = () => {
    ...
    canvas.toBlob(blob => {
      const filename = `${datasetName || 'game'}_frames_frame_${selectedCategory || 'action'}_${selectedAction || 'shot'}_time_${ts}_x${currentPosition.x.toFixed(1)}_y${currentPosition.y.toFixed(1)}.png`;
  ...
  const downloadClip = async () => {
    ...
    recorder.onstop = () => {
      ...
      const fn = `${datasetName || 'game'}_clips_clip_${selectedCategory || 'action'}_${selectedAction || 'shot'}_time_${center.toFixed(2)}_x${currentPosition.x.toFixed(1)}_y${currentPosition.y.toFixed(1)}.webm`;
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = fn;
      a.click();
  ...
  // Function to open data in Soccer Analysis format
  const handleOpenInAnalysis = () => {
    ...
    // Soccer pitch dimensions and goal positions used in the analysis dashboard
  ...
  const handleGoBack = () => navigate('/analysis-soccer');
...
export default ManualTaggingSoccer;