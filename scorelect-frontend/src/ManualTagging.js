...
const ManualTagging = () => {
  ...
  // Tag categories and actions for GAA
  const tagCategories = [
  ...
  const handleVideoError = (e) => {
    ...
    const supportedFormatsList = supportedFormats
      ...
      .join(', ');
    let errorMessage = "Error loading video: The format may not be supported by your browser.";
    if (supportedFormatsList) {
      errorMessage += ` Please upload or convert to ${supportedFormatsList}.`;
    } else {
      errorMessage += " Your browser may not support HTML5 video.";
    }
  ...
  const captureFrame = () => {
    if (videoFile && videoRef.current && canvasRef.current && videoReady) {
      ...
      try {
        ...
        setCurrentFrameData(frameDataUrl);
        return frameDataUrl;
      } catch (error) {
  ...
  // --- New: downloadFrame & downloadClip functions ---
  const downloadFrame = () => {
    const video = videoRef.current;
    const canvas = canvasRef.current;
    ...
    ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
    canvas.toBlob(blob => {
      const filename = `frames_${datasetName}/action_${ts}s.png`;
  ...
  const downloadClip = async () => {
    ...
    recorder.onstop = () => {
      ...
      const filename = `clips_${datasetName}/clip_${center.toFixed(2)}s.webm`;
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = filename;
      a.click();
  ...
  const handleSave = () => {
    ...
    if (tags.length === 0) return alert("No tags to save");
  ...
  // Export tags JSON
  const handleExport = () => {
    if (tags.length === 0) return alert("No tags to export");
    ...
    <PageContainer maxWidth="xl">
      ...
      <Grid container spacing={3}>
              ...
              <>
                {videoFile ? (
                  ...
                  >
                    ...
                    Your browser does not support HTML5 video. Please upload an MP4 file.
                  </video>
                ) : (
                  <YouTube
                  ...
                  <ControlsRow>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      <Select
                        ...
                        size="small"
                        sx={{ backgroundColor: 'rgba(0,0,0,0.5)', color: '#fff', height: 30, '& .MuiSelect-select': { p: '2px 8px' } }}
                      >
          ...
          <TimelineContainer>
            {tags.map((tag, i) => (
              <TimelineMark
                key={i}
                category={tag.category}
                active={Math.abs(tag.timestamp - currentTime) < 0.5}
        ...
        <Grid item xs={12} lg={4}>
          <Tabs value={activeTab} onChange={(e, v) => setActiveTab(v)} variant="fullWidth"
            sx={{ backgroundColor: '#1a1a1a', borderRadius: '8px 8px 0 0', '& .MuiTabs-indicator': { backgroundColor: '#5e2e8f' } }}
          >
            <StyledTab label="Tagged Events" />
            <StyledTab label="Quick Tags" />
          </Tabs>
          <SectionCard sx={{ borderRadius: '0 0 8px 8px', maxHeight: '600px', overflowY: 'auto' }}>
            {activeTab === 0 ? (
              <>
                  ...
                  tags.map((tag, i) => (
                    <Box key={i} sx={{ p: 1, mb: 1, borderRadius: 1, backgroundColor: 'rgba(25,25,25,0.5)', border: '1px solid #333' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Box>
                          <IconButton size="small" onClick={() => handleJumpToTag(tag.timestamp)} sx={{ color: '#5e2e8f' }}><PlayArrowIcon fontSize="small" /></IconButton>
                          <IconButton size="small" onClick={() => handleDeleteTag(tag.id)} sx={{ color: '#dc3545' }}><DeleteIcon fontSize="small" /></IconButton>
                        </Box>
                      </Box>
                      ...
                      <Typography variant="body2" sx={{ mt: 1, color: '#aaa' }}>{tag.team}: {tag.player || 'No player'}</Typography>
                      {tag.notes && <Typography variant="body2" sx={{ mt: 0.5, color: '#ddd', fontSize: '0.8rem' }}>{tag.notes}</Typography>}
                    </Box>
                  ))
                )}
              </>
                        ...
                        {cat.actions.map(act => (
                          <TagButton
                            ...
                            colorscheme={
                              ...
                              cat.id === 'Set Pieces' ? 'warning' :
                              cat.id === 'Turnovers' ? 'secondary' : 'primary'
                            }
                            onClick={() => {
          ...
          <Box sx={{ mt: 2, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            <ActionButton color="success" onClick={handleSave} startIcon={<SaveIcon />} fullWidth>
              Save Dataset
            </ActionButton>
      ...
      >
        <DialogTitle sx={{ borderBottom: '1px solid #444' }}>
          ...
          <IconButton aria-label="close" onClick={handleCloseTagDialog} sx={{ position: 'absolute', right: 8, top: 8, color: '#aaa' }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
      ...
      {/* Save Dialog */}
      <Dialog open={saveDialogOpen} onClose={() => setSaveDialogOpen(false)} PaperProps={{ sx: { backgroundColor: '#222', color: '#fff', borderRadius: 2, maxWidth: '400px', width: '100%' } }}>
        <DialogTitle sx={{ borderBottom: '1px solid #444' }}>
          Save Dataset
          <IconButton aria-label="close" onClick={() => setSaveDialogOpen(false)} sx={{ position: 'absolute', right: 8, top: 8, color: '#aaa' }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
      ...
      {/* Help Dialog */}
      <Dialog open={helpDialogOpen} onClose={() => setHelpDialogOpen(false)} PaperProps={{ sx: { backgroundColor: '#222', color: '#fff', borderRadius: 2, maxWidth: '500px', width: '100%' } }}>
        <DialogTitle sx={{ borderBottom: '1px solid #444' }}>
          Video Format Help
          <IconButton aria-label="close" onClick={() => setHelpDialogOpen(false)} sx={{ position: 'absolute', right: 8, top: 8, color: '#aaa' }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
